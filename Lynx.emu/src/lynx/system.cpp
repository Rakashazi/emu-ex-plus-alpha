...
static void Load(GameFile* gf)
{
 try
 {
  ...
  lynxie->mMikie->mikbuf.set_sample_rate(44100, 60);
...
void Lynx_SetPixelFormat(MDFN_PixelFormat format)
{
...
{
  lynxie->mMikie->mikbuf.set_sample_rate(rate, 60);
}

long Lynx_GetSoundRate()
{
	return lynxie->mMikie->mikbuf.left()->sample_rate();
}

void Lynx_SetLowpassFilter(bool on)
{
...
//  printf("%d ", gSystemCycleCount - lynxie->mMikie->startTS);
 ...
 if(espec->SoundBuf)
 {
  ...
  espec->SoundBufSize = lynxie->mMikie->mikbuf.read_samples(espec->SoundBuf, espec->SoundBufMaxSize * 2) / 2; // divide by nr audio chn
 }
 else
