...
class ChainResampler : public Resampler {
	...
	typedef SubResampler * (*CreateSinc)(unsigned div, float rollOffStart, float rollOffWidth, SysDDec gain);
	...
	void downinitAddSincResamplers(SysDDec ratio, float outRate,
			...
			unsigned bigSincMul, unsigned smallSincMul, SysDDec gain);
	
	template<class Sinc>
	static SubResampler * createSinc(unsigned div, float rollOffStart, float rollOffWidth, SysDDec gain) {
...
std::size_t ChainResampler::upinit(const long inRate, const long outRate, const std::size_t periodSize) {
	...
	/*{
		...
		SysDDec ratio_2c = ratio * div_2c / SmallSinc::MUL;
		...
		if (cost_2c < get1ChainCost(ratio, rollOff)) {
			...
			const SysDDec ratio1_3c = ratio * div1_3c / SmallSinc::MUL;
			const int div2_3c = get3ChainRatio2(ratio1_3c, rollOff) * SmallSinc::MUL / ratio1_3c + 0.5f;
			const SysDDec ratio2_3c = ratio1_3c * div2_3c / SmallSinc::MUL;
