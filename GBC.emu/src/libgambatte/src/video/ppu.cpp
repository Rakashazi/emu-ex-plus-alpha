...
static void doFullTilesUnrolledDmg(PPUPriv &p, int const xend, PixelType *const dbufline,
...

		{
			...
			if (!lcdcObjEn(p)) {
				...
				do {
					...
					PixelType const *const spPalette = p.spPalette + (attrib >> 2 & 4);
					PixelType *d = dst + pos;
...
static void doFullTilesUnrolledCgb(PPUPriv &p, int const xend, PixelType *const dbufline,
		unsigned char const *const tileMapLine, unsigned const tileline, unsigned tileMapXpos) {
	int xpos = p.xpos;
	...
	do {
		...
		if (int(p.spriteList[nextSprite].spx) < xpos + 8) {
			...
			do {
				PixelType const *const bgPalette = p.bgPalette + (nattrib & 7) * 4;
...

		{
			...
			PixelType const *const bgPalette = p.bgPalette + (attrib & 7) * 4;
			...
			if (!lcdcObjEn(p)) {
				...
				do {
					...
					unsigned spword        = p.spwordList[i];
					PixelType const *const spPalette = p.spPalette + (sattrib & 7) * 4;
