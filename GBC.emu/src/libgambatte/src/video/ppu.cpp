...
namespace M3Loop {
	static void doFullTilesUnrolledDmg(PPUPriv &p, const int xend, PixelType *const dbufline,
		...
		do {
			...
			if (static_cast<int>(p.spriteList[nextSprite].spx) < xpos + 8) {
				...
				PixelType *dst = dbufline + xpos - 8;
				PixelType *const dstend = dst + n;
				xpos += n;
			...
			{
				PixelType *const dst = dbufline + (xpos - 8);
				...
				if (!(p.lcdc & 2)) {
					...
					do {
						...
						const PixelType *const spPalette = p.spPalette + (attrib >> 2 & 4);
						PixelType *d = dst + pos;
	...
	static void doFullTilesUnrolledCgb(PPUPriv &p, const int xend, PixelType *const dbufline,
			const unsigned char *const tileMapLine, const unsigned tileline, unsigned tileMapXpos) {
		int xpos = p.xpos;
		...
		do {
			...
			if (static_cast<int>(p.spriteList[nextSprite].spx) < xpos + 8) {
				...
				do {
					const PixelType *const bgPalette = p.bgPalette + (nattrib & 7) * 4;
			...
			
			{
				...
				const PixelType *const bgPalette = p.bgPalette + (attrib & 7) * 4;
				...
				if (!(p.lcdc & 2)) {
					...
					do {
						...
						unsigned spword        = p.spwordList[i];
						const PixelType *const spPalette = p.spPalette + (sattrib & 7) * 4;
						
						if (!((attrib | sattrib) & bgenmask)) {
							unsigned char  *const idt = idtab + pos;
							PixelType *const   d =   dst + pos;
	...
	static void doFullTilesUnrolled(PPUPriv &p) {
		...
		PixelType *const dbufline = p.framebuf.fbline();
	...
	static void plotPixel(PPUPriv &p) __attribute__ ((hot));
	static void plotPixel(PPUPriv &p) {
		const int xpos = p.xpos;
		const unsigned tileword = p.tileword;
		PixelType *const fbline = p.framebuf.fbline();
		...
		PixelType pixel = p.bgPalette[twdata + (p.attrib & 7) * 4];
