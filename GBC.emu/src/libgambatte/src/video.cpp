...
void LCD::setDmgPalette(PixelType palette[], PixelType const dmgColors[], unsigned data) {
...
, m1IrqStatReg_(0)
{
	...
	for (std::size_t i = 0; i < sizeof dmgColorsRgb_ / sizeof dmgColorsRgb_[0]; ++i)
		setDmgPaletteColor(i, (3 - (i & 3)) * 85 * 0x010101ul);
...
void LCD::refreshPalettes() {
	if (ppu_.cgb()) {
		for (unsigned i = 0; i < 8 * 8; i += 2) {
			ppu_.bgPalette()[i >> 1] = mapGbcColor( bgpData_[i] |  bgpData_[i + 1] << 8);
			ppu_.spPalette()[i >> 1] = mapGbcColor(objpData_[i] | objpData_[i + 1] << 8);
		}
	} else {
		setDmgPalette(ppu_.bgPalette()    , dmgColorsRgb_    ,  bgpData_[0]);
		setDmgPalette(ppu_.spPalette()    , dmgColorsRgb_ + 4, objpData_[0]);
		setDmgPalette(ppu_.spPalette() + 4, dmgColorsRgb_ + 8, objpData_[1]);
	}
}
...
void LCD::updateScreen(bool const blanklcd, unsigned long const cycleCounter) {
	...
	if (blanklcd && ppu_.frameBuf().fb()) {
		unsigned long color = ppu_.cgb() ? mapGbcColor(0xFFFF) : dmgColorsRgb_[0];
...
static void doCgbColorChange(unsigned char *pdata,
		PixelType *palette, unsigned index, unsigned data) {
	pdata[index] = data;
	index >>= 1;
	palette[index] = mapGbcColor(pdata[index * 2] | pdata[index * 2 + 1] << 8);
}
...
void LCD::setVideoBuffer(PixelType *videoBuf, std::ptrdiff_t pitch) {
...
void LCD::setDmgPaletteColor(unsigned index, unsigned long rgb32)
{
	if(usingRGB565Color) // convert to RGB565
		dmgColorsRgb_[index] = ((rgb32 & 0xFF) >> 3) | (((rgb32 & 0xFF00) >> (8+2)) << 5) | (((rgb32 & 0xFF0000) >> (16+3)) << 11);
	else
		dmgColorsRgb_[index] = rgb32;
	refreshPalettes();
}
