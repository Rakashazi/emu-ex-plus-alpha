...
void OutZ80(uint16 port, uint8 value)
{
#ifdef NEOPOP_DEBUG
...
uint16 Z80_getReg(uint8 reg)
{
	uint16* r = (uint16*)&Z80_regs;
	return r[reg];
}

void Z80_setReg(uint8 reg, uint16 value)
{
	uint16* r = (uint16*)&Z80_regs;
	r[reg] = value;
}
...
#ifdef NEOPOP_DEBUG
char* Z80_disassemble(uint16* pc_in)
{
	int bcnt, i;
	uint16 pc = *pc_in;
