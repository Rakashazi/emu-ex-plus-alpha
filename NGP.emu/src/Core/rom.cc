...
void rom_bootHacks(void)
{
	if (MATCH_CATALOG(278, 6) && rom.data[0x1c] == 0x94)
	{
		logMsg("Card Fighters 2 English patch boot hack");
		// run 3 frames with extra CPU sync for Card Fighters 2 English patch intro
		frameskip_active = 1;
		iterateTimes(3, i)
		{
			unsigned gotVBL = 0;
