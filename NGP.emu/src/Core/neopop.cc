...
void emulate(void)
{
	...
	//system_message("start loop");
	while(!gotVBL)
	{
		gotVBL = updateTimers<1>(TLCS900h_interpret());
	}
}
...
void emulate_debug(bool dis_TLCS900h, bool dis_Z80)
{
	...
	if (dis_Z80)
	{
		...
		{
			char* s;
			uint16 pc = Z80_getReg(Z80_REG_PC);
			uint16 store_pc = pc;
	...
	//==================
	{
		...
		gotVBL = updateTimers(TLCS900h_interpret());
