...
void interrupt(uint8 index)
{
	...
	pc = le32toh(*(uint32*)(ram + 0x6FB8 + (index * 4)));
...
static void updateClocks(uint32 cputicks)
{
	//=======================
...
unsigned int updateTimers(uint32 cputicks)
{
	...
	if(!runZ80InHInt && Z80ACTIVE)
	{
		...
		if(Z80_regs.ICount > 64)
			ExecZ80Cycles(&Z80_regs);
	}
#endif

	unsigned int frameDone = 0;
	//increment H-INT timer
	timer_hint += cputicks;

	//=======================

	/*uint hLines = timer_hint / TIMER_HINT_RATE;
		if(hLines > 1)
			logMsg("%d lines", hLines);*/
	...
	if (timer_hint >= TIMER_HINT_RATE)
	{
		...
		if(runZ80InHInt && Z80ACTIVE)
		{
			...
			//ExecZ80Cycles(&Z80_regs);
			Z80_regs.ICount = 300;//(float)TIMER_HINT_RATE * .75;
			ExecZ80Cycles(&Z80_regs);
		}
#endif
	}

	updateClocks(cputicks);

	return frameDone;
	//=======================
}

template unsigned int updateTimers<1>(uint32 cputicks);
template unsigned int updateTimers<0>(uint32 cputicks);
