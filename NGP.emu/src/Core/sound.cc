...
uint8 dacBufferL[DAC_BUFFERSIZE];
...
static uint32 VolTable[16];
static uint32 UpdateStep;	//Number of steps during one sample.
...
static uint16 sample_chip_tone(void)
{
	int i;
...
static uint16 sample_chip_noise(void)
{
#if 0
	int i;
#endif
...
void WriteSoundChip(SoundChip* chip, uint8 data)
{
	//Command
	if (data & 0x80)
	{
