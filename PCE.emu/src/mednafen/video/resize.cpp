...
bool MDFN_ResizeSurface(const MDFN_Surface *src, const MDFN_Rect *src_rect, const MDFN_Rect *LineWidths, MDFN_Surface *dest, const MDFN_Rect *dest_rect)
{
 ...
 src_x_inc = (SysDDec)src_rect->w / dest_rect->w;
 src_y_inc = (SysDDec)src_rect->h / dest_rect->h;
 ...
 for(int32 y = 0; y < dest_rect->h; y++)
 {
  ...
  if(LineWidths)
  {
   ...
   src_x_inc = (SysDDec)LineWidths[INT(src_y)].w / dest_rect->w;
  ...
  for(int32 x = 0; x < dest_rect->w; x++)
  {
   ...
   if(INT(src_x_inc) > 1)
   {
    for(SysDDec subx = 1; subx < (INT(src_x_inc) - 1); subx++)
    {
   ...
   if(INT(src_y_inc) > 1)
   {
    for(SysDDec suby = 1; suby < (INT(src_y_inc) - 1); suby++)
    {
