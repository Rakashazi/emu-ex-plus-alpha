...
bool SMPC_HasMultitap(unsigned sport)
{
	assert(sport < 2);
	return SPorts[sport] != nullptr;
}
...
void SMPC_LoadNV(IG::FileIO& s)
{
 s.rewind();
 auto valid = s.get<uint8>();
 if(!valid)
	 return;
 RTC.Valid = valid;
 s.read(RTC.raw, sizeof(RTC.raw));
...
void SMPC_SaveNV(IG::FileIO& s)
{
 s.rewind();
 s.put<uint8>(RTC.Valid);
 s.write(RTC.raw, sizeof(RTC.raw));
 s.write(SaveMem, sizeof(SaveMem));
}
