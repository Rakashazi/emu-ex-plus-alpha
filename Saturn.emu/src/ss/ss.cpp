  ...
  if(IsWrite)
  {
   if(sizeof(T) != 1 || (A & 1))
   {
     ...
     *brp = (uint8)DB;
     ...
     EmuEx::gSystem().onBackupMemoryWritten(0x1);
    }
   }
  }
  else
...
#undef RLTDAT
 ...
 MDFN_MidSync(espec); // finish audio output
 //
 //
 //
 SMPC_UpdateOutput();
...
void SaveBackupRAM(IG::FileIO& brs)
{
 brs.write(BackupRAM, sizeof(BackupRAM), 0);
}

void LoadBackupRAM(IG::FileIO& brs)
{
 brs.read(BackupRAM, sizeof(BackupRAM), 0);
 if(!std::ranges::equal(BackupRAM | std::views::take(sizeof(BRAM_Init_Data)), BRAM_Init_Data))
 {
  for(unsigned i = 0; i < 0x40; i++)
   BackupRAM[i] = BRAM_Init_Data[i & 0x0F];
 }
