...
static const struct model_s vic20models[] = {
    { MACHINE_SYNC_PAL,  NO_EXTRA_RAM, "chargen"},
    { MACHINE_SYNC_NTSC, NO_EXTRA_RAM, "chargen"},
    { MACHINE_SYNC_NTSC, BLOCK_1 | BLOCK_2, "chargen"}, /* SuperVIC */
    { MACHINE_SYNC_NTSC, NO_EXTRA_RAM, "chargen-jp"},    /* vic-1001 (japanese VIC20) */
};

/* ------------------------------------------------------------------------- */

static int vic20model_get_temp(int video, int ramblocks, const char *chargen)
{
    int i;
    ...
    for (i = 0; i < VIC20MODEL_NUM; ++i) {
        if ((vic20models[i].video == video)
            ...
            && (chargen ? !strcmp(vic20models[i].chargenname, chargen) : 1)) {
            return i;
        }
    }
...
int vic20model_get(void)
{
    ...
    return vic20model_get_temp(video, ramblocks, chargen);
}
