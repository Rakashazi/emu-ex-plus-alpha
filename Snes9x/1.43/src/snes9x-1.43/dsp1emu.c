...
SysDDec CosTable2[INCR];
SysDDec SinTable2[INCR];
...
void C4CalcWireFrame()
{
  ...
  if (abs(C4WFXVal)>abs(C4WFYVal)){
    ...
    C4WFYVal=(256*(int)C4WFYVal)/abs(C4WFXVal);
  ...
  if (C4WFYVal!=0) {
    ...
    C4WFXVal=(256*(int)C4WFXVal)/abs(C4WFYVal);
...
static short Op04Angle;
static short Op04Radius;
static short Op04Sin;
static short Op04Cos;

static const short DSP1_MulTable[256] = {
...
static const short DSP1_SinTable[256] = {
...
static short DSP1_Sin(short Angle)
{
	if (Angle < 0) {
...
static short DSP1_Cos(short Angle)
{
	if (Angle < 0) {
...
//   tanval2 = (-Op02AZS)/256;
   ...
   if (ObjPZ2<0)
   {
      ...
      //Op06S=(unsigned short)((SysDDec)(256.0*((SysDDec)Op02LES)/(-ObjPZ2)));
   }
   else
   {
      Op06H=0;
