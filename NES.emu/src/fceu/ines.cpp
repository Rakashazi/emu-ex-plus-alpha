...
uint8 *CHRRAM = GameMemBlock+sizeofWRAM+sizeofMapperExRAM;
...
#define CHRRAMSize CHRRAMSize_ines
...
static void iNESPower(void)
{
	...
	if(type && type!=13 && type!=96)
	{
		...
		for(x=0;x<8;x++)
		{
			const char *tak[] = { "CBL0", "CBL1", "CBL2", "CBL3", "CBL4", "CBL5", "CBL6", "CBL7" };
			//sprintf(tak,"CBL%d",x);
			AddExState(&CHRBankList[x], 2, 1,tak[x]);
		}
	}
