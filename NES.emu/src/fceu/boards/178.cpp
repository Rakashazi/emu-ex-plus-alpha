...
{
	using namespace Board178;
  info->Power=M178Power;
  info->Close=M178Close;
  GameStateRestore=Board178::StateRestore;

  Board178::WRAMSIZE=8192;
  Board178::WRAM=(uint8*)FCEU_gmalloc(Board178::WRAMSIZE);
  SetupCartPRGMapping(0x10,Board178::WRAM,Board178::WRAMSIZE,1);
  if(info->battery)
  {
    info->SaveGame[0]=Board178::WRAM;
    info->SaveGameLen[0]=Board178::WRAMSIZE;
  }
  AddExState(Board178::WRAM, Board178::WRAMSIZE, 0, "WRAM");

  AddExState(&Board178::StateRegs, ~0, 0, 0);
}
