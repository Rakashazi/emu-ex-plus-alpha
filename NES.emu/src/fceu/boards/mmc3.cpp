...
void FixMMC3CHR(int V)
{
 ...
 if(mwrap) mwrap(BoardMMC3::A000B);
}
...
void GenMMC3_Init(CartInfo *info, int prg, int chr, int wram, int battery)
{
 ...
 {
  mmc3opts|=1;
  BoardMMC3::WRAM=(uint8*)FCEU_gmalloc(wrams);
  SetupCartPRGMapping(0x10,BoardMMC3::WRAM,wrams,1);
  AddExState(BoardMMC3::WRAM, wrams, 0, "MMC3WRAM");
 }

 if(battery)
 {
  mmc3opts|=2;
  info->SaveGame[0]=BoardMMC3::WRAM;
...
void Mapper74_Init(CartInfo *info)
{
 ...
 cwrap=M74CW;
 CHRRAMSize=2048;
 BoardMMC3::CHRRAM=(uint8*)FCEU_gmalloc(CHRRAMSize);
 SetupCartCHRMapping(0x10, BoardMMC3::CHRRAM, CHRRAMSize, 1);
 AddExState(BoardMMC3::CHRRAM, CHRRAMSize, 0, "CHRR");
}
...
void Mapper116_Init(CartInfo *info)
{
 ...
 BoardMMC3::CHRRAM = (uint8*)FCEU_gmalloc(8192);
 SetupCartCHRMapping(0x10, BoardMMC3::CHRRAM, 8192, 1);
