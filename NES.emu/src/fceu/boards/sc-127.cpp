...
{
	using namespace BoardSC_127;
  info->Reset=UNLSC127Reset;
  info->Power=UNLSC127Power;
  info->Close=UNLSC127Close;
  GameHBIRQHook=UNLSC127IRQ;
  GameStateRestore=BoardSC_127::StateRestore;
  WRAMSIZE=8192;
  BoardSC_127::WRAM=(uint8*)FCEU_gmalloc(WRAMSIZE);
  SetupCartPRGMapping(0x10,BoardSC_127::WRAM,WRAMSIZE,1);
  AddExState(BoardSC_127::WRAM, WRAMSIZE, 0, "WRAM");
  AddExState(&BoardSC_127::StateRegs, ~0, 0, 0);
}
