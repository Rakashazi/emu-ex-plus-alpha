...
static void FixCache(int a,int V)
{
  ...
  switch(a&0x07)
  {
    ...
    case 0x04:FreqCache[w]&=~0x00030000;FreqCache[w]|=(V&3)<<16;
              ...
              break;
    case 0x07:EnvCache[w]=(SysDDec)(V&0xF)*576716;break;
  }
}
  ...
  for(P=7;P>=7-((IRAM[0x7F]>>4)&7);P--)
  {
    if((IRAM[0x44+(P<<3)]&0xE0) && (IRAM[0x47+(P<<3)]&0xF))
    {
...

      {
        ...
        inc=(SysLDDec)(FSettings.SndRate<<15)/((SysLDDec)freq*21477272/((SysLDDec)0x400000*c*45));
      }
...
void Mapper19_Init(CartInfo *info)
{
  ...
  AddExState(BoardN106::WRAM, 8192, 0, "WRAM");
  AddExState(IRAM, 128, 0, "IRAM");
  AddExState(N106_StateRegs, ~0, 0, 0);

  if(info->battery)
  {
    info->SaveGame[0]=BoardN106::WRAM;
