...
{
	using namespace BoardKonami_Qtai;
  info->Power=M190Power;
  info->Close=M190Close;
  GameStateRestore=BoardKonami_Qtai::StateRestore;

  MapIRQHook=VRC5IRQ;
  //PPU_hook=Mapper190_PPU;

  BoardKonami_Qtai::CHRRAM=(uint8*)FCEU_gmalloc(BoardKonami_Qtai::CHRSIZE);
  SetupCartCHRMapping(0x10,BoardKonami_Qtai::CHRRAM,BoardKonami_Qtai::CHRSIZE,1);
  AddExState(BoardKonami_Qtai::CHRRAM, BoardKonami_Qtai::CHRSIZE, 0, "CHRRAM");

  BoardKonami_Qtai::WRAM=(uint8*)FCEU_gmalloc(BoardKonami_Qtai::WRAMSIZE);
  SetupCartPRGMapping(0x10,BoardKonami_Qtai::WRAM,BoardKonami_Qtai::WRAMSIZE,1);
  AddExState(BoardKonami_Qtai::WRAM, BoardKonami_Qtai::WRAMSIZE, 0, "WRAM");

  if(info->battery)
  {
    info->SaveGame[0] = BoardKonami_Qtai::WRAM;
    info->SaveGameLen[0] = BoardKonami_Qtai::WRAMSIZE - 4096;
  }

  AddExState(&BoardKonami_Qtai::StateRegs, ~0, 0, 0);
}
