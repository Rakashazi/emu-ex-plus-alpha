...
#define CHRRAMSize CHRRAMSize_unif
...
static int DoMirroring(FCEUFILE *fp)
{
	...
	{
		const char *stuffo[6]={"Horizontal","Vertical","$2000","$2400","\"Four-screen\"","Controlled by Mapper Hardware"};
		if(t<6)
...
static int InitializeBoard(void)
{
	...
	while(bmap_unif[x].name)
	{
		if(!strcmp((char *)sboardname,(char *)bmap_unif[x].name))
		{
			if(!malloced[16])
			{
				if(bmap_unif[x].flags & BMCFLAG_16KCHRR)
					CHRRAMSize = 16384;
				else
					CHRRAMSize = 8192;
			...
			if(bmap_unif[x].flags&BMCFLAG_FORCE4)
				mirrortodo=4;
			MooMirroring();
			bmap_unif[x].init(&UNIFCart);
			return(1);
		}
		x++;
	}
