...
{
	if (GameInfo->type==GIT_VSUNI)
		FCEUI_VSUniToggleDIP(execcmd-(int)EMUCMD_VSUNI_TOGGLE_DIP_0);
}

static void CommandEmulationSpeed(void)
{
	FCEUD_SetEmulationSpeed((int)EMUSPEED_SLOWEST+(execcmd-(int)EMUCMD_SPEED_SLOWEST));
}
...
static void CommandUsePreset(void)
{
	FCEUI_UseInputPreset(execcmd-(int)EMUCMD_MISC_USE_INPUT_PRESET_1);
}
