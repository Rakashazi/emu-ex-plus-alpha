...
static void DoLine(void)
{
	...
	if(scanline >= 8 && scanline < 224 + 8)
	{
		...
		if((PPU[1]>>5)==0x7)
		{
			...
			for(x=0;x<=255;x++)
			{
				...
				outLine[x] = nativeCol[(target[x]&0x3f)|0xc0];
			}
		}
		else if(PPU[1]&0xE0)
			for(x=0;x<=255;x++)
			{
				...
				outLine[x] = nativeCol[(target[x])|0x40];
			}
		else
			for(x=0;x<=255;x++)
			{
				//*(uint32 *)&target[x<<2]=((*(uint32*)&target[x<<2])&0x3f3f3f3f)|0x80808080;
				outLine[x] = nativeCol[(target[x]&0x3f)|0x80];
			}
	}
	...
	//memcpy(XBuf+(scanline<<8), target, 256);
	scanline++;
	if(scanline<240)
	{
		ResetRL(lineBuffer);//XBuf+(scanline<<8));
	}
	X6502_Run(16);
}
		...
		for(n=63;n>=0;n--,spr++)
		{
			...
			if(ns<maxsprites)
			{
...

				{
					...
					//*(uint32 *)&SPRBUF.b[ns<<2]=*(uint32 *)&dst;
					SPRBUF.dw[(ns<<2)/4]=*(uint32 *)&dst;
				}
