...
#define rate_adjust(x) (rate==49716?x:(e_uint32)((SysDDec)(x)*clk/72/rate + 0.5))        /* added 0.5 to round the value*/
...
makeAdjustTable (void)
{
  ...
  for (i = 1; i < 128; i++)
    AR_ADJUST_TABLE[i] = (e_uint16) ((SysDDec) (1 << EG_BITS) - 1 - (1 << EG_BITS) * log ((SysDDec)i) / log (128.));
}
...
makeDB2LinTable (void)
{
  ...
  {
    DB2LIN_TABLE[i] = (e_int16) ((SysDDec) ((1 << DB2LIN_AMP_BITS) - 1) * pow (10, -(SysDDec) i * DB_STEP / 20));
...
makePmTable (void)
{
  ...
  for (i = 0; i < PM_PG_WIDTH; i++)
    pmtable[i] = (e_int32) ((SysDDec) PM_AMP * pow (2, (SysDDec) PM_DEPTH * sin (2.0 * PI * i / PM_PG_WIDTH) / 1200));
}
...
makeAmTable (void)
{
  ...
  for (i = 0; i < AM_PG_WIDTH; i++)
    amtable[i] = (e_int32) ((SysDDec) AM_DEPTH / 2 / DB_STEP * (1.0 + sin (2.0 * PI * i / PM_PG_WIDTH)));
}
...
static SysDDec attacktime[16][4] = {
  {0, 0, 0, 0},
  ...
  for (RM = 0; RM < 16; RM++)
    for (RL = 0; RL < 4; RL++)
    {
      if(RM == 0)
        ...
        attacktable[RM][RL] = (e_uint32) ((SysDDec) (1 << EG_DP_BITS) / (attacktime[RM][RL] * 3579545 / 72000));
  ...
  for (RM = 0; RM < 16; RM++)
    for (RL = 0; RL < 4; RL++)
      if(RM == 0)
        ...
        decaytable[RM][RL] = (e_uint32) ((SysDDec) (1 << EG_DP_BITS) / (decaytime[RM][RL] * 3579545 / 72000));
#endif
...
{
#define S2E(x) e_uint32(SL2EG((e_int32)(x/SL_STEP))<<(EG_DP_BITS-EG_BITS))
...
void moocow(OPLL* opll, InSample *buf, e_int32 len, int shift)
{
 while(len > 0)
 {
...
template void moocow(OPLL* opll, FCEU_SoundSample *buf, e_int32 len, int shift);
template void moocow(OPLL* opll, FCEU_SoundSample2 *buf, e_int32 len, int shift);
...
OPLL_calc (OPLL * opll)
{
  ...
  opll->out = (e_int16) (((SysDDec) opll->next * (opll->opllstep - opll->oplltime)
                          + (SysDDec) opll->prev * opll->oplltime) / opll->opllstep);
