...
void utilGBAFindSave(const uint8_t *rom, const int size)
{
...
void utilUpdateSystemColorMaps(int colorDepth, int redShift, int greenShift, int blueShift, bool lcd)
{
  ...
  case 16: {
      ...
      if (lcd)
      	gbafilter_pal(systemColorMap.map16, 0x10000, redShift, greenShift, blueShift);
    } break;
  case 32: {
      for (int i = 0; i < 0x10000; i++) {
      ...
      if (lcd)
      	gbafilter_pal32(systemColorMap.map32, 0x10000, redShift, greenShift, blueShift);
    } break;
  }
}
