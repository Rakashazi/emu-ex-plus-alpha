...
#else
static void gfxDrawTextScreen(uint8_t vram[0x20000], uint16_t, uint16_t, uint16_t, uint32_t *,
		const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette);
#endif
static void gfxDrawRotScreen(uint8_t vram[0x20000], uint16_t,
			     uint16_t, uint16_t,
			     uint16_t, uint16_t,
			     uint16_t, uint16_t,
			     uint16_t, uint16_t,
			     ...
			     uint32_t*,
			     const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette);
static void gfxDrawRotScreen16Bit(uint8_t vram[0x20000], uint16_t,
				  uint16_t, uint16_t,
				  uint16_t, uint16_t,
				  uint16_t, uint16_t,
				  uint16_t, uint16_t,
...
static void gfxDrawRotScreen256(uint8_t vram[0x20000], uint16_t,
				uint16_t, uint16_t,
				uint16_t, uint16_t,
				uint16_t, uint16_t,
				uint16_t, uint16_t,
...
static void gfxDrawRotScreen16Bit160(uint8_t vram[0x20000], uint16_t,
				     uint16_t, uint16_t,
				     uint16_t, uint16_t,
				     uint16_t, uint16_t,
				     uint16_t, uint16_t,
...
static inline void gfxDrawTextScreen(uint8_t vram[0x20000], uint16_t control, uint16_t hofs, uint16_t vofs,
				     uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t *palette)
{
  const uint8_t *charBase = &vram[((control >> 2) & 0x03) * 0x4000];
  const uint16_t *screenBase = (uint16_t*)&vram[((control >> 8) & 0x1f) * 0x800];
...
static inline void gfxDrawRotScreen(uint8_t vram[0x20000], uint16_t control,
...
static inline void gfxDrawRotScreen16Bit(uint8_t vram[0x20000], uint16_t control,
					 uint16_t x_l, uint16_t x_h,
					 uint16_t y_l, uint16_t y_h,
					 uint16_t pa,  uint16_t pb,
					 uint16_t pc,  uint16_t pd,
					 ...
					 uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC)
{
  const uint16_t *screenBase = (uint16_t*)&vram[0];
  int prio = ((control & 3) << 25) + 0x1000000;
  int sizeX = 240;
  int sizeY = 160;
...
static inline void gfxDrawRotScreen256(uint8_t vram[0x20000], uint16_t control,
				       uint16_t x_l, uint16_t x_h,
				       uint16_t y_l, uint16_t y_h,
				       uint16_t pa,  uint16_t pb,
				       uint16_t pc,  uint16_t pd,
				       ...
				       const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT, const uint16_t *palette)
{
  const uint8_t *screenBase = (DISPCNT & 0x0010) ? &vram[0xA000] : &vram[0x0000];
  int prio = ((control & 3) << 25) + 0x1000000;
  int sizeX = 240;
  int sizeY = 160;
...
static inline void gfxDrawRotScreen16Bit160(uint8_t vram[0x20000], uint16_t control,
					    uint16_t x_l, uint16_t x_h,
					    uint16_t y_l, uint16_t y_h,
					    uint16_t pa,  uint16_t pb,
					    uint16_t pc,  uint16_t pd,
					    ...
					    uint32_t* line, const uint16_t VCOUNT, const uint16_t MOSAIC, const uint16_t DISPCNT)
{
  const uint16_t *screenBase = (DISPCNT & 0x0010) ? (uint16_t*)&vram[0xa000] : (uint16_t*)&vram[0];
  int prio = ((control & 3) << 25) + 0x1000000;
  int sizeX = 160;
  int sizeY = 128;
      ...
      if (a0 & 0x0100) {
        ...
        if ((t >= 0) && (t < fieldY)) {
          ...
          if ((sx < 240) || startpix) {
            ...
            // int t2 = t - (fieldY >> 1);
            int rot = (a1 >> 9) & 0x1F;
            const uint16_t* OAM = (uint16_t*)oam;
            int dx = READ16LE(&OAM[3 + (rot << 4)]);
            if (dx & 0x8000)
              dx |= 0xFFFF8000;
            int dmx = READ16LE(&OAM[7 + (rot << 4)]);
