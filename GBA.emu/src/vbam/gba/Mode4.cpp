...
void mode4RenderLine(MixColorType *lineMix, GBALCD &lcd, const GBAMem::IoMem &ioMem)
{
  ...
  if(lcd.layerEnable & 0x400) {
    int changed = lcd.gfxBG2Changed;

    if(lcd.gfxLastVCOUNT > VCOUNT)
      changed = 3;

    gfxDrawRotScreen256(lcd.vram, ioMem.BG2CNT, ioMem.BG2X_L, ioMem.BG2X_H, ioMem.BG2Y_L, ioMem.BG2Y_H,
    		ioMem.BG2PA, ioMem.BG2PB, ioMem.BG2PC, ioMem.BG2PD,
    		lcd.gfxBG2X, lcd.gfxBG2Y, changed,
                        lcd.line2, VCOUNT, MOSAIC, DISPCNT, palette);
  }
